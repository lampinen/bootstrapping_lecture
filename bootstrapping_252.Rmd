...
density_similarity_conceptual_plot_data = expand.grid(x=seq(0, 4, 0.01),
                                                      y=seq(0, 4, 0.01)) %>%
  mutate(group_1 = exp(-((x-2)^2 +(y-3)^2)/8) * exp(-((x-2/y)^2 +(y-1/x)^2)/2), #These are definitely not proper distributions
         group_2 = exp(-((x)^2 +(y)^2)/8) * exp(-((x/2)^2 +(y/2-1/x)^2)/2)) %>% 
...
ggplot(density_similarity_conceptual_plot_data,
       aes(x=x, y=y, z=value, color=group)) +
