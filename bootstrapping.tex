...
\uncover<2>{
However, both tests benefit from normality, so you should transform your data if that makes it more normal. 
}
\end{frame}
...
\only<3->{
\begin{itemize}[<+(2)->]
\item If you can make the data better through experiment design or transformation, do that first (it can only improve things). 
\item Samples should not be ``too'' small ($\gtrsim 20$ points, otherwise you \textbf{need} stronger assumptions that probably will be mostly easily expressed in a parametric framework).  
\item Should be many resamples ($R \approx 1000$ at least, come on, it's the 21st century, modern laptops are more powerful than 20th century supercomputers).
\item Non-parametrics are still generally \textbf{not} robust to non-independence, so still need to use mixed(/hierarchical) models, and bootstrapping within these models must respect dependence structure.
\end{itemize}
}
...
\item Nonparametrics aren't a panacea, but they cure quite a lot. 
\end{itemize}
\end{frame}
